...
#
# Dữ liệu: doanh số bán lẻ và dịch vụ ăn uống Mỹ (Census MARTS), CPI-U (BLS), dân số (BEA NIPA).
...
def so_sanh_thang(y: pd.Series, thang_truoc: str, thang_sau: str) -> float:
    """Tăng trưởng (tỷ lệ) từ tháng trước sang tháng sau."""
    return float(y[pd.Timestamp(thang_sau)] / y[pd.Timestamp(thang_truoc)] - 1)
...
def tang_truong_thuc_dau_nguoi(y: pd.Series, cpi: pd.Series, dan_so: pd.Series, tu_nam: int, den_nam: int) -> float:
    """Tăng trưởng thực trên đầu người."""
    return tang_truong(y, tu_nam, den_nam)
...
def boxcox_nguoc(w, lam: float, sigma2=0.0) -> np.ndarray:
    """Đổi ngược Box-Cox về thang gốc."""
    w = np.asarray(w, dtype=float)
    if lam == 0:
        return np.exp(w)
    goc = lam * w + 1
    return np.sign(goc) * np.abs(goc) ** (1 / lam)
