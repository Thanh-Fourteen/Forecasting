...
def sua_crossing(Q: np.ndarray) -> np.ndarray:
    """Quantile mức cao không được nhỏ hơn quantile mức thấp: sắp xếp lại từng dòng."""
...
def quantile_tu_phan_du(R: pd.DataFrame, muc=MUC) -> np.ndarray:
    """Quantile = dự báo điểm + quantile của phần dư NGOÀI MẪU (3 tháng backtest ngay trước tháng đang dự báo).

    Không giả định phân phối chuẩn; phần dư ngoài mẫu mới phản ánh sai số thật khi dự báo.
    """
    e = (R["y"] - R["yhat"]).to_numpy()
    thang = R["ds"].dt.to_period("M")
    Q = np.full((len(R), len(muc)), np.nan)
    for m in thang.unique():
        cal = (thang < m) & (thang >= m - SO_THANG_PHAN_DU)
        if cal.sum() == 0:
            continue
        dong = (thang == m).to_numpy()
        Q[dong] = R["yhat"].to_numpy()[dong, None] + np.quantile(e[cal.to_numpy()], muc)[None, :]
    return Q
...
def crps_mau(y, mau) -> float:
    """CRPS từ mẫu: trung bình |X − y| trừ nửa trung bình |X − X'| (X, X' là hai phần tử của mẫu)."""
    y = np.asarray(y, float)
    X = np.asarray(mau, float)
    xa = np.abs(X - y[:, None]).mean(axis=1)
    Xs = np.sort(X, axis=1)
    m = X.shape[1]
    i = np.arange(1, m + 1)
    trai_phai = (2 * (Xs * (2 * i - m - 1)).sum(axis=1)) / m ** 2       # = trung bình |X − X'| trên mọi cặp
    return float(np.mean(xa - 0.5 * trai_phai))
