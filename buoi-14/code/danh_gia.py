...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: 4.227 chuỗi M4 theo ngày (Monash), doanh số UCI Online Retail II.
#
...
BASELINE = {"trung bình": bl_trung_binh, "naive": bl_naive, "drift": bl_drift}
...
def mase(y: np.ndarray, d: np.ndarray, hoc: np.ndarray, m: int = M) -> float:
    """MASE: MAE chia cho MAE của naive mùa vụ, chuẩn hoá theo chính đoạn đang chấm."""
    mau = _mau_so_scaled(y, m)
    return float("nan") if not mau else mae(y, d) / mau


def rmsse(y: np.ndarray, d: np.ndarray, hoc: np.ndarray, m: int = M) -> float:
    """RMSSE (M5): RMSE chia cho RMSE của naive mùa vụ, chuẩn hoá theo chính đoạn đang chấm."""
    mau = _mau_so_scaled(y, m, binh_phuong=True)
