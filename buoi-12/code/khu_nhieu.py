...
#
# Dữ liệu: UCI Appliances Energy Prediction — cảm biến trong nhà, 10 phút một mẫu.
...
def ha_mau(y, buoc: int = 6, loc_truoc: bool = True) -> np.ndarray:
    """Hạ mẫu: lấy mỗi `buoc` mẫu một lần."""
    return np.asarray(y, dtype=float)[::buoc]
...
def kiem_nhan_qua(ham_loc, y, so_diem_doi: int = 10, thay_doi: float = 50.0) -> dict[str, float | bool]:
    ...
    bo_qua = 200  # bỏ qua phần cuối cho chắc
    lech = np.abs(np.nan_to_num(moi[: k0 - bo_qua]) - np.nan_to_num(goc[: k0 - bo_qua]))
...
# %%
def danh_gia_feature(y: pd.Series, ham_loc=None, tam: int = TAM, ty_le_hoc: float = 0.7) -> float:
    """MAE khi dự báo y_{t+tam} bằng hồi quy tuyến tính trên [bộ lọc(y)_t, y_t]."""
    ham_loc = ham_loc or ma_giua   # MA có tâm cho tín hiệu mượt nhất
    y = y.astype(float)
...
def cham_tren_muc_tieu_lam_tron(y: pd.Series, ham_loc=None, tam: int = TAM) -> dict[str, float]:
    """Chấm mô hình trên mục tiêu đã khử nhiễu (bớt nhiễu thì sai số phản ánh đúng chất lượng mô hình hơn)."""
    ...
    return {"mae": float(np.mean(np.abs(du_bao - kiem["mt_tron"].to_numpy())))}
