...
#
# Bản ĐÃ SỬA. Dữ liệu: GDP thực Mỹ theo quý (BEA), chỉ số sản lượng công nghiệp theo tháng (FRB G.17),
...
def kiem_dinh(y, co_xu_huong: bool = False) -> dict[str, float]:
    """ADF (H0: có nghiệm đơn vị) và KPSS (H0: dừng) trên cùng dạng xác định: 'c' hoặc 'ct'.

    KPSS chỉ tra bảng 0,01–0,1 nên p bị cắt; `kpss_ngoai_bang` ghi lại điều đó.
    """
    y = np.asarray(y, dtype=float)
    y = y[~np.isnan(y)]
    dang = "ct" if co_xu_huong else "c"
    adf = adfuller(y, regression=dang, autolag="AIC", result_object=True)
    with warnings.catch_warnings(record=True) as ghi:
        warnings.simplefilter("always")
        kq_kpss = kpss(y, regression=dang, nlags="auto", result_object=True)
    return {"adf_stat": float(adf.statistic), "adf_p": float(adf.pvalue), "kpss_stat": float(kq_kpss.statistic),
            "kpss_p": float(kq_kpss.pvalue), "kpss_ngoai_bang": any("look-up table" in str(g.message) for g in ghi)}


def ket_luan(y, co_xu_huong: bool = False) -> str:
    """Bốn tổ hợp ADF × KPSS (heuristic, không phải định lý)."""
    kq = kiem_dinh(y, co_xu_huong)
    adf_bac_bo = kq["adf_p"] < MUC          # bác bỏ nghiệm đơn vị
    kpss_bac_bo = kq["kpss_p"] < MUC        # bác bỏ tính dừng
    if adf_bac_bo and not kpss_bac_bo:
        return "dừng quanh xu hướng" if co_xu_huong else "dừng"
    if not adf_bac_bo and kpss_bac_bo:
        return "không dừng — cần sai phân"
    if not adf_bac_bo and not kpss_bac_bo:
        return "không đủ bằng chứng (dữ liệu ngắn hoặc gần nghiệm đơn vị)"
    return "hai kiểm định mâu thuẫn — xem lại dạng xác định và phương sai đổi"


def so_lan_sai_phan(y, toi_da: int = 2) -> int:
    """Số lần sai phân theo KPSS (như FPP/ndiffs): sai phân tiếp khi KPSS còn bác bỏ tính dừng."""
    y = pd.Series(np.asarray(y, dtype=float)).dropna()
    for d in range(toi_da + 1):
        if kiem_dinh(y)["kpss_p"] >= MUC:
            return d
        y = y.diff().dropna()
    return toi_da
...
def bang_bon_chuoi(n: int = 500, seed: int = 42) -> pd.DataFrame:
    ...
    for ten, y in sinh_bon_chuoi(n, seed).items():
        c, ct = kiem_dinh(y), kiem_dinh(y, co_xu_huong=True)
        hang.append({"chuỗi": ten, "ACF(1)": round(acf_tu_viet(y, 1)[1], 3),
                     "ADF c": round(c["adf_p"], 3), "KPSS c": round(c["kpss_p"], 3),
                     "ADF ct": round(ct["adf_p"], 3), "KPSS ct": round(ct["kpss_p"], 3),
                     "kết luận (c)": ket_luan(y), "kết luận (ct)": ket_luan(y, True),
                     "d theo KPSS": so_lan_sai_phan(y)})
