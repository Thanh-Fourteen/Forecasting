...
def ve_acf(ax, y, so_tre: int, dai: float, ten: str, pacf_thay=False) -> None:
...
def hinh_nhieu_trang(so_lan: int = 1000, so_tre: int = 20, n: int = 500, seed: int = 2026) -> dict:
    ...
    b.set_title(f"Trung bình {dem.mean():.2f} cột vượt; {np.mean(dem >= 1):.0%} số chuỗi có ít nhất 1", fontsize=9)
...
def hinh_sai_phan_thua() -> dict:
    ...
    for cot, ten in enumerate(["nhiễu trắng", "random walk"]):
        ...
        truc[0, cot].set_title(f"{ten} sau khi sai phân — sd {sp.std(ddof=1):.2f} (trước {y.std(ddof=1):.2f})", fontsize=9)
...
def hinh_that() -> dict:
    ...
    truc[0, 1].set_title("sai phân log (×100): dao động quanh 0,7%/quý, vài cú sốc lớn", fontsize=9)
    ve_acf(truc[1, 0], log_gdp.to_numpy(), 24, tt.dai_nhieu_trang(len(gdp)), "ACF log GDP: giảm rất chậm")
    ve_acf(truc[1, 1], tang.to_numpy(), 24, tt.dai_nhieu_trang(len(tang)), "ACF sai phân log: vài trễ đầu còn tương quan")
...
def hinh_luot_thue() -> dict:
    y = tt.doc_luot_thue()
    ngay = y.resample("D").sum(min_count=24).dropna()
    fig, (a, b) = plt.subplots(1, 2, figsize=(10, 3.4))
    ve_acf(a, y, 200, tt.dai_nhieu_trang(int(y.notna().sum())), "ACF lượt thuê theo giờ (trễ tới 200)")
    ve_acf(b, ngay, 40, tt.dai_nhieu_trang(len(ngay)), "ACF tổng theo ngày (trễ tới 40)")
    fig.suptitle("Cùng dữ liệu, hai tần suất: theo giờ thấy mùa vụ 24 và 168; theo ngày chỉ còn mùa vụ 7",
                 fontsize=10, fontweight="bold")
    fig.tight_layout()
