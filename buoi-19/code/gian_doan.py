...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: 2.674 chuỗi bán phụ tùng ô tô theo tháng, 1/1998 → 3/2002 (Car Parts, Monash).
...
def tsb(y: np.ndarray, alpha_d: float = 0.1, alpha_p: float = 0.1) -> float:
    ...
    if not (y > 0).any():
        return 0.0
    co_ban = y > 0
    xac_suat = ses(co_ban[co_ban].astype(float), alpha_p)
    luong = ses(y[co_ban], alpha_d)
...
def danh_gia(kq: pd.DataFrame, Y: pd.DataFrame, mo_hinh=MO_HINH) -> pd.DataFrame:
    """Chỉ số của dự báo 1 tháng tới, trung bình qua các chuỗi."""
    k1 = kq[kq["buoc_h"] == 1]
    ...
    for m in mo_hinh:
        ...
        co_ban = that > 0
        hang.append({"mô hình": m, "MAE": float(np.abs(e).mean()),
                     "MAPE": float(np.mean(np.abs(e[co_ban]) / that[co_ban]) * 100), "ME": float(e.mean())})
    return pd.DataFrame(hang).set_index("mô hình")
...
def chon_mo_hinh(bang: pd.DataFrame) -> str:
    """Mô hình dùng để đặt hàng."""
    return str(bang["MAE"].idxmin())
