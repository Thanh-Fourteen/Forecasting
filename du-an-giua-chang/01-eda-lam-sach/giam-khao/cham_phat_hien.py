...
#
# Mỗi lỗi: 5 điểm = **3 đúng loại + 1 đúng tệp/cột + 1 đúng khoảng thời gian (±3 ngày)**.
# Báo lỗi không có thật: **−2 điểm** mỗi cái (tối đa trừ 6) — để không ai "bắn đại cho trúng".
...
def cham(bao_cao: dict, dap_an: dict) -> dict:
    ...
    for muc in bao_cao.get("loi", []):
        ...
        g = that[loai]
        diem = 3
        dung_tep = _chuan(muc.get("tep", "")) == _chuan(g["tep"])
        dung_cot = g["cot"] == "*" or _chuan(muc.get("cot", "")) == _chuan(g["cot"])
        if dung_tep and dung_cot:
            diem += 1
        dung_khoang = _trong_khoang(muc, g)
        if dung_khoang:
            diem += 1
        chi_tiet.append({"ma": g["ma"], "loại": loai, "điểm": diem, "đúng tệp/cột": dung_tep and dung_cot,
