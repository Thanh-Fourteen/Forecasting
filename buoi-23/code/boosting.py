...
THAM_SO = {"objective": "regression", "n_estimators": 400, "learning_rate": 0.03,
...
def chia_cv(b_hoc: pd.DataFrame, so_fold: int = 3) -> list[tuple[np.ndarray, np.ndarray]]:
    """Chia phần học thành `so_fold` fold (KFold, xáo trộn, seed cố định)."""
    from sklearn.model_selection import KFold
    return list(KFold(so_fold, shuffle=True, random_state=SEED).split(b_hoc))
