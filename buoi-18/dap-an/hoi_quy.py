# %% [markdown]
# # Buổi 18 — Hồi quy chuỗi thời gian và hồi quy động (ĐÁP ÁN)
#
# Dữ liệu: hành khách hàng không EU (Eurostat) và sản lượng công nghiệp Mỹ (G.17) theo tháng; tải điện ERCOT theo giờ 2024 (EIA-930)
...
def he_so_ip(y: pd.Series, X: pd.DataFrame) -> dict:
    """Hệ số của sản lượng công nghiệp khi sai số là SARIMA(0,1,1)(0,1,1)12 (hồi quy động). Mùa vụ đã do sai phân mùa
    vụ lo, nên chỉ đưa ip vào."""
    from statsmodels.tsa.statespace.sarimax import SARIMAX
    kq = SARIMAX(y, exog=X[["ip"]], order=(0, 1, 1), seasonal_order=(0, 1, 1, 12)).fit(disp=False)
    e = kq.resid.to_numpy()[13:]                      # bỏ 1 + 12 điểm đầu (sai phân chưa đủ dữ liệu)
    return {"hệ số ip": float(kq.params["ip"]), "p": float(kq.pvalues["ip"]), "Ljung-Box p": ljung_box_p(e)}
...
KHAI_TET = True             # khai Tết cho Prophet
