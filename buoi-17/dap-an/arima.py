...
#
# Bản ĐÃ SỬA. Dữ liệu: chỉ số sản lượng công nghiệp Mỹ theo tháng (Federal Reserve G.17, đã khử mùa vụ),
...
MUA_VU_ARIMA = 12   # chu kỳ mùa vụ đưa vào auto-ARIMA
...
def kiem_phan_du(mh, so_tre: int = 24) -> dict:
    """Ljung–Box trên phần dư (bỏ các điểm đầu mà sai phân chưa có đủ dữ liệu). 'dat' = p ≥ 0,05."""
    p, q, P, Q, m, d, D = mh.model_["arma"]
    bo = d + D * max(m, 1)
    lb = ljung_box(mh.model_["residuals"][bo:], so_tre, p + q + P + Q)
    return {**lb, "dat": lb["p"] >= 0.05}


def mo_hinh_tu_chon(y: np.ndarray, order=(0, 1, 1), seasonal_order=(0, 1, 1)):
    """Mô hình đọc từ ACF/PACF của chuỗi đã sai phân (mục 4.4): mặc định SARIMA(0,1,1)(0,1,1)12."""
...
def bang_ung_vien(y: np.ndarray, cac_bac, seasonal_order=(0, 0, 0)) -> pd.DataFrame:
    ...
    for bac in cac_bac:
        ...
        hang.append({"mô hình": ten_mo_hinh(mh), "AICc": mh.model_["aicc"], "Ljung–Box p": kiem_phan_du(mh)["p"]})
    tu_dong = auto_arima(y)
    hang.append({"mô hình": "auto → " + ten_mo_hinh(tu_dong), "AICc": tu_dong.model_["aicc"],
                 "Ljung–Box p": kiem_phan_du(tu_dong)["p"]})
