...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: 366 chuỗi du lịch theo tháng (Tourism, Monash), hành khách hàng không EU theo tháng (Eurostat).
...
def ses_loc(y: np.ndarray, alpha: float) -> tuple[np.ndarray, float]:
    """Trả (giá trị khớp cho y_1..y_{n-1}, mức cuối). Mức ban đầu = y_0."""
    muc = float(y[0])
    khop = np.empty(len(y) - 1)
    for t in range(1, len(y)):
        muc = alpha * y[t] + (1 - alpha) * muc          # cập nhật mức bằng y_t
        khop[t - 1] = muc
    return khop, muc
...
# %%
def holt_winters(y: pd.Series, h: int, kieu: str = "cong", level: list[int] | None = None) -> pd.DataFrame:
