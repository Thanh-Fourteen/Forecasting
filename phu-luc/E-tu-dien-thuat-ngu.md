...
| WRMSSE (weighted RMSSE) | WRMSSE | RMSSE của từng chuỗi, cộng có trọng số theo doanh thu gần đây. Là điểm chính thức của cuộc thi M5. | Mã A RMSSE 1, 75% doanh thu; mã B RMSSE 0, 25% → WRMSSE = 0,75. | 23 |
...
| summing matrix S | ma trận tổng S | Ma trận 0/1 cho biết mỗi chuỗi là tổng của những chuỗi đáy nào; mọi chuỗi = S × các chuỗi đáy. | Cả nước, Bắc, Nam: S = [[1, 1], [1, 0], [0, 1]]. | 28 |
| coherent / incoherent | khớp / không khớp | Dự báo khớp khi mỗi chuỗi cấp trên đúng bằng tổng dự báo các chuỗi bên dưới nó. | Bắc 40 + Nam 50 nhưng cả nước 95 → không khớp, lệch 5. | 28 |
| middle-out | middle-out | Dự báo ở một cấp giữa, cộng lên cho cấp trên và chia xuống cho cấp dưới. | Dự báo 8 bang, cộng ra cả nước, chia xuống khu vực theo tỷ lệ. | 28 |
| temporal hierarchy | phân cấp theo thời gian | Cùng một chuỗi ở nhiều tần suất (quý, năm): năm phải bằng tổng 4 quý. | Dự báo năm 100, bốn quý cộng ra 103 → hoà giải cho khớp. | 28 |
...
| decision tree, random forest | cây quyết định, rừng ngẫu nhiên | Cây hỏi liên tiếp các câu có/không về đầu vào; mỗi lá trả trung bình các dòng học rơi vào đó. Rừng là trung bình nhiều cây, mỗi cây học trên một phần dữ liệu chọn ngẫu nhiên. | "Hôm qua > 100?" → có: đoán 120; không: đoán 40. || 15, 22 |
| extrapolation | ngoại suy | Dự báo ra ngoài khoảng giá trị đã thấy khi học. Cây và LightGBM không làm được: lá chỉ trả trung bình nhãn đã thấy. | Doanh số học luôn dưới 300; chuỗi đang tăng nên tháng tới cần 310, cây vẫn đoán ≤ 300. | 22 |
...
| learning rate | học suất (learning rate) | Tỷ lệ của mỗi cây (hoặc mỗi bước cập nhật) được cộng vào mô hình. Nhỏ thì học chậm, cần nhiều bước hơn, ít học thuộc nhiễu hơn. | Cây nói "+4", học suất 0,5 → cộng 2. | 23, 29 |
| partial dependence (PD) | partial dependence | Dự báo trung bình khi đặt một đặc trưng bằng từng giá trị cho mọi dòng, giữ nguyên các cột khác. Mô tả mô hình, không phải tác động thật. | Đặt giá = £1 cho mọi dòng → trung bình 22 món; £2 → 21 món. | 23, 39 |
| monotone constraint | ràng buộc đơn điệu | Lệnh cấm mô hình tăng (hoặc giảm) dự báo khi một đặc trưng tăng. | Ràng buộc −1 cho giá: giá tăng thì dự báo không được tăng. | 23 |
...
| epoch, early stopping | epoch, dừng sớm | Epoch là một lượt mô hình học qua toàn bộ dữ liệu huấn luyện. Dừng sớm: ngưng học (hoặc ngưng thêm cây) khi sai số trên tập kiểm định thôi giảm. | Sai số kiểm định sau epoch 1–5: 9; 7; 6; 6,2; 6,5 → dừng, giữ epoch 3. | 23, 29 |
...
| conformity score | điểm conformal | Con số đo "dự báo sai cỡ nào" ở một điểm, dùng để dựng khoảng conformal. Hay dùng nhất: sai số bỏ dấu. | Thật 52, dự báo 47 → điểm 5. | 26 |
| conditional coverage | coverage có điều kiện | Tỷ lệ phủ tính riêng trong một nhóm (giờ ô nhiễm nặng, mùa đông…). Coverage chung đạt 90% vẫn có thể chỉ phủ 70% ở nhóm khó. | Cả năm phủ 90%, nhưng những giờ PM2.5 > 150 chỉ phủ 72%. | 26 |
| peaks-over-threshold (POT) | peaks-over-threshold | Cách dự báo giá trị cực đoan: chỉ giữ các đợt vượt một ngưỡng cao, rồi khớp một phân phối cho phần vượt. | 86 năm nhiệt độ, giữ 89 đợt nóng trên 39 °C. | 25 |
| generalized Pareto distribution (GPD) | phân phối Pareto tổng quát (GPD) | Phân phối dùng cho phần vượt ngưỡng. Tham số hình dạng $\xi$ < 0: đuôi có trần; = 0: đuôi giảm như hàm mũ; > 0: đuôi dày. | Nhiệt độ vượt 39 °C: $\xi$ ≈ −0,25, đuôi có trần. | 25 |
| return level / return period | mức lặp lại / chu kỳ lặp lại | Mức N năm là mức trung bình N năm mới bị vượt một lần; N là chu kỳ lặp lại. Không có nghĩa là đúng N năm mới xảy ra một lần. | Mức 10 năm 41,4 °C: mỗi năm có khoảng 10% khả năng vượt. | 25 |
...
| no pooling / complete pooling / partial pooling | gộp thông tin (không / hoàn toàn / một phần) | Nhiều nhóm có thể mỗi nhóm tự ước lượng, dùng chung một số, hoặc gộp một phần (kéo nhóm ít dữ liệu về gần mức chung). | Cửa hàng mới bán 20 cái/ngày (mới 3 ngày), cả chuỗi 10 cái/ngày → gộp một phần ước lượng khoảng 14. | 27 |
...
| posterior predictive | phân phối dự báo hậu nghiệm | Dự báo Bayes: rút tham số từ posterior rồi rút số liệu tương lai theo tham số đó; gộp lại được cả độ bất định của tham số lẫn của số liệu. | Rút tốc độ bán 0,6; 0,9; 0,7… rồi mỗi lần rút số bán ~ Poisson → phân phối số bán tháng tới. | 27 |
| MCMC, NUTS | MCMC, NUTS | Cách rút mẫu từ posterior bằng một chuỗi bước đi ngẫu nhiên; NUTS là cách đi có định hướng mà PyMC dùng mặc định. | 4 chuỗi × 1.000 bước → 4.000 mẫu của tham số. | 27 |
| centered / non-centered parametrization | tham số hoá centered / non-centered | Hai cách viết cùng một mô hình phân cấp: θ ~ Normal(μ, τ) hoặc θ = μ + τ·z với z ~ Normal(0, 1). Cách thứ hai tránh "cái phễu" khi τ nhỏ. | 8 trường: centered 312 divergence, non-centered 0. | 27 |
| shrinkage | co rút | Ước lượng của một nhóm ít dữ liệu bị kéo về mức chung của mọi nhóm; càng ít dữ liệu càng bị kéo mạnh. | 35 mã chỉ có 3 tháng, toàn 0 món: không gộp đoán 0; gộp một phần đoán 0,22 món/tháng, đúng bằng trung bình thật 12 tháng sau. | 27 |
| HSGP (Hilbert space GP) | HSGP | Cách xấp xỉ Gaussian process bằng một số hữu hạn hàm sin/cos, để chi phí tăng tuyến tính thay vì theo lập phương số điểm. | 671 ngày, 40 hàm cơ sở thay vì ma trận 671 × 671. | 27 |
| BSTS (Bayesian structural time series) | BSTS | Mô hình state space ghép từ các thành phần (mức, xu hướng, mùa vụ, hồi quy), ước lượng bằng Bayes. | Mức + xu hướng + mùa tuần + mùa năm cho lượt thuê xe. | 27 |
