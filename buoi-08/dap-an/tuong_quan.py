...
#
# Bản ĐÃ SỬA. Dữ liệu: tải điện ERCOT theo giờ (EIA-930), nhiệt độ Dallas + Houston (Open-Meteo ERA5),
...
def tuong_quan_chuoi(a: pd.Series, b: pd.Series) -> dict[str, float | str]:
    """Tương quan trên MỨC và trên SAI PHÂN, kèm R²/t/DW của hồi quy mức."""
    a, b = a.dropna(), b.dropna()
    chung = a.index.intersection(b.index)
    a, b = a[chung], b[chung]
    da, db = a.diff().dropna(), b.diff().dropna()
    hq = hoi_quy_don(a, b)
    kq = {"r_muc": float(a.corr(b)), "r_sai_phan": float(da.corr(db)), **hq}
    nghi_gia = abs(kq["r_muc"]) > 0.8 and hq["dw"] < 1 and abs(kq["r_sai_phan"]) < 0.3
    kq["ket_luan"] = ("nghi TƯƠNG QUAN GIẢ: mức tương quan cao, phần dư tự tương quan mạnh (DW thấp), "
                      "sai phân xong gần như hết" if nghi_gia else "không thấy dấu hiệu tương quan giả rõ ràng")
    return kq
...
def do_tre_dan_dat(x, y, so_tre: int = 12, bac: int | None = 48) -> dict[str, float]:
    """Độ trễ mà x dẫn y, ĐO SAU prewhitening (bac=None để xem CCF thô)."""
    a, b = (x, y) if bac is None else loc_prewhiten(x, y, bac)
...
def granger_hai_chieu(x: pd.Series, y: pd.Series, so_tre: int = 4) -> dict[str, object]:
    ...
    if p_xy < 0.05 and p_yx < 0.05:
        ket_luan = ("CẢ HAI CHIỀU cùng có ý nghĩa — không thể đọc thành nhân quả; thường do biến gây nhiễu chung "
                    "hoặc mùa vụ chung")
    elif p_xy < 0.05:
        ket_luan = "quá khứ của x giúp dự báo y (Granger), KHÔNG kết luận x gây ra y"
    elif p_yx < 0.05:
        ket_luan = "quá khứ của y giúp dự báo x (Granger), KHÔNG kết luận y gây ra x"
    else:
