...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: giá dầu WTI và giá xăng New York Harbor theo ngày (EIA); GDP thực, việc làm, sản lượng công nghiệp, nhà khởi công
...
def hang_dong_lien_ket(L: pd.DataFrame, k_ar_diff: int = 2) -> int:
    """Số quan hệ cointegration giữa các chuỗi."""
    return 0
...
def nowcast(du_lieu: dict, q: pd.Period, k: int, chi_bao=DUNG_CHI_BAO) -> dict:
    ...
    v = VINTAGE_CUOI                          # số liệu mới nhất, đầy đủ nhất
