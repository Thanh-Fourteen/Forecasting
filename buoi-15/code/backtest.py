...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: tải điện ERCOT (Texas) theo giờ năm 2024 (EIA-930), 414 chuỗi M4 theo giờ (Monash).
#
...
def uoc_luong_sai_so(y: pd.Series, tao_mo_hinh: Callable, so_cua_so: int = 28,
                     buoc: int = 24) -> float:
    """MAE ước lượng trên phần học: để riêng 20% số dòng làm tập kiểm, học trên phần còn lại."""
...
def chon_va_bao_cao(chuoi: dict[str, np.ndarray], h: int = H_M4) -> pd.DataFrame:
    """Đoạn cuối B của mỗi chuỗi: tune w của 'trộn', chọn phương pháp tốt nhất, báo cáo MASE của nó.
...
def diebold_mariano(e1, e2, h: int = 1, ham_mat_mat: str = "tuyet_doi", hieu_chinh: bool = True) -> KetQuaDM:
    ...
    d_t = g(e1_t) − g(e2_t); thống kê = trung bình của d chia sai số chuẩn của trung bình đó.
    """
    ...
    v = float(np.var(d)) / n                     # phương sai của trung bình
