...
PRIOR = {"mu_sd": 10.0, "tau_sd": 10.0} # log tốc độ bán trung bình ~ Normal(0, mu_sd); độ chênh giữa mã ~ HalfNormal(tau_sd)
...
THANH_PHAN_GP = ("xu_huong", "nam", "tuan")   # thành phần kernel của GP
...
def dung_duoc(cd: dict, nguong: dict = NGUONG) -> bool:
    """Posterior dùng được khi các chuỗi Markov hội tụ."""
    return cd["r_hat_max"] < nguong["r_hat"]
