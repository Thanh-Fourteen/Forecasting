# %% [markdown]
# # Buổi 1 — Đánh giá một dự báo
#
# Bài toán: mỗi thứ Hai 00:00, dự báo điện tiêu thụ từng giờ của 7 ngày tới (tầm 168 giờ) cho một hộ gia đình ở
# Sceaux (Pháp). Đơn vị: kWh mỗi giờ (= công suất trung bình kW trong giờ đó).
...
MOC = "2010-01-04"  # thứ Hai đầu tiên của năm 2010
...
def du_bao_tuan_truoc(lich_su: pd.Series, tam: int = TAM) -> np.ndarray:
    """Giờ này tuần trước. Lịch sử phải đã điền, đủ ít nhất một tuần."""
...
def du_bao_gio_truoc(lich_su: pd.Series, tam: int = TAM) -> np.ndarray:
    """Giữ nguyên giờ cuối cùng đã biết."""
    return np.full(tam, lich_su.iloc[-1])


def du_bao_trung_binh(lich_su: pd.Series, tam: int = TAM) -> np.ndarray:
    """Trung bình mọi giờ đã biết."""
    return np.full(tam, lich_su.mean())


# %%
def du_bao_cuon(chuoi: pd.Series, moc: str = MOC, tam: int = TAM) -> pd.DataFrame:
    """Bảng dự báo theo giờ từ mốc trở đi: cột `goc` (gốc dự báo), `y` (số đo thật) và cột dự báo."""
    bang = bang_lich(chuoi)
    cac_goc = pd.date_range(pd.Timestamp(moc), chuoi.index[-1] - pd.Timedelta(hours=tam - 1), freq=f"{TAM}h")
    phan = []
    for goc in cac_goc:
        ...
        phan.append(pd.DataFrame({
            ...
            "bảng lịch": du_bao_bang_lich(bang, thoi_gian),
        }, index=thoi_gian))
    return pd.concat(phan)


def danh_gia(chuoi: pd.Series, moc: str = MOC, tam: int = TAM) -> pd.Series:
    """MAE của các phương pháp trên phần sau mốc, xếp từ tốt tới kém."""
