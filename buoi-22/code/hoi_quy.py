...
def dac_trung(df: pd.DataFrame, h: int, so_lag: int = SO_LAG) -> pd.DataFrame:
    """Đặc trưng để dự báo z ở ngày ds với tầm h (chiến lược direct).

    lag_k = z của ngày ds − k, k = 1 … so_lag. muc = trung bình các lag; các lag trừ đi muc (chuẩn hoá theo
    chuỗi). thu = thứ trong tuần của ds.
    """
    g = df.groupby("unique_id", sort=False)["z"]
    lag = pd.concat({f"lag_{k}": g.shift(k) for k in range(1, so_lag + 1)}, axis=1)
    muc = lag.mean(axis=1)
...
def du_bao_cay(y: np.ndarray, h: int, so_lag: int = 7) -> np.ndarray:
    """Cây quyết định recursive cho một chuỗi."""
    from sklearn.tree import DecisionTreeRegressor
    y = np.asarray(y, float)
    X, nhan = bang_lag(y, so_lag)
    cay = DecisionTreeRegressor(min_samples_leaf=3, random_state=SEED).fit(X, nhan)
    return du_bao_de_quy(cay, y, h, so_lag)
