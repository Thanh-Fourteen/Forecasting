...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: trạm Nội Bài (NOAA GHCNh, 2024), Open-Meteo Hà Nội (trạm hàng xóm),
...
def doc_noi_bai(tep=None) -> pd.DataFrame:
    ...
    Parquet đã có sẵn kiểu dữ liệu, cứ lấy nguyên cột là xong.
    """
...
def so_sanh_dien(y: pd.Series, hang_xom: pd.Series | None = None, cach=None, seed: int = 0) -> pd.DataFrame:
    """MAE của từng cách điền khi che ngẫu nhiên 10% số điểm."""
    cach = cach or CACH_DIEN
    that = y.dropna()
    kieu = {"che điểm 10%": che_diem(that, 0.10, seed)}
    hang = []
...
def lam_sach(bang: pd.DataFrame, cot: str = "temperature", gioi_han: int = 6, mac_ket: int = 36,
    ...
    Quy tắc:
      ...
      4. bỏ đoạn cảm biến đứng yên quá lâu (`mac_ket` bước);
      5. điền mọi lỗ còn lại để chuỗi liên tục.
    """
    ...
    thieu = sach.isna()
    ket_qua = dien_tuyen_tinh(sach)
    return pd.DataFrame({cot: ket_qua, "da_dien": thieu, "nghi_ngo": nghi,
                         "lo_dai_bo_trong": pd.Series(False, index=day_du.index)},
                        index=day_du.index)
