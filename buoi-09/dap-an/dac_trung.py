...
#
# Bản ĐÃ SỬA. Dữ liệu: 48.000 chuỗi M4 theo tháng (Monash), doanh số bán lẻ UCI Online Retail II.
...
def tuong_quan_kho_de(bang: pd.DataFrame) -> pd.DataFrame:
    """Tương quan giữa đặc trưng và sai số thật — báo CẢ Pearson lẫn Spearman, CẢ ba thước đo.

    Đây là chỗ dễ kết luận sai nhất của buổi: MASE chia cho sai số của chính seasonal naive nên nó
    "chuẩn hoá mất" đúng phần khó mà entropy đang đo.
    """
    hang = []
    for dac_trung in ("entropy_pho", "he_so_bien_thien", "do_manh_mua_vu"):
        for do in ("smape_snaive", "mase_snaive", "mase_naive1_snaive"):
            x, y = bang[dac_trung].to_numpy(float), bang[do].to_numpy(float)
            ok = np.isfinite(x) & np.isfinite(y)
            hang.append({"đặc trưng": dac_trung, "thước đo": do,
...
def de_xuat_chien_luoc(bang: pd.DataFrame, nguong_entropy: float = 0.666, nguong_mua_vu: float = 0.4) -> pd.Series:
    """Chuỗi entropy cao và mùa vụ yếu thì **dùng baseline**, đừng tốn công tune.

    Ngưỡng mặc định lấy từ phân vị 80% của entropy trên mẫu 4.000 chuỗi M4 tháng (0,666)."""
    kho = (bang["entropy_pho"] >= nguong_entropy) & (bang["do_manh_mua_vu"] < nguong_mua_vu)
    return pd.Series(np.where(kho, "dùng baseline", "đáng đầu tư mô hình"), index=bang.index, name="chien_luoc")
...
def phan_cum_dtw(chuoi: dict[str, np.ndarray], so_cum: int = 4, chuan_hoa: bool = True,
