...
#
# Điểm xuất phát (có chỗ cố tình sai). Dữ liệu: giá BTC/USDT theo giờ 2023–2024 (Binance Public Data); tỷ giá JPY/USD theo ngày 2000–2024 (Federal Reserve H.10).
...
import tv
from tv.backtest import diebold_mariano  # noqa: F401 — kiểm định DM (buổi 15)
...
def du_bao_gia(gia: pd.Series, moc: str = MOC, seed: int = 0) -> pd.DataFrame:
    """Mạng nơ-ron (MLP) dự báo giá ngày mai từ 10 ngày trước. Chuẩn hoá giá về [0, 1], chia 80% học / 20% kiểm."""
    from sklearn.model_selection import train_test_split
    from sklearn.neural_network import MLPRegressor
    from sklearn.preprocessing import MinMaxScaler
    X, y, idx = cua_so(gia)
    sc = MinMaxScaler().fit(gia.to_numpy().reshape(-1, 1))
    def chuan(a):
        return sc.transform(a.reshape(-1, 1)).reshape(a.shape)
    Xh, Xk, yh, _, _, ik = train_test_split(X, y, idx, test_size=0.2, random_state=seed)
    mh = MLPRegressor(hidden_layer_sizes=(64, 64), max_iter=3000, random_state=seed).fit(chuan(Xh), chuan(yh))
    du_bao = sc.inverse_transform(mh.predict(chuan(Xk)).reshape(-1, 1)).ravel()
    kq = pd.DataFrame({"y": gia.reindex(ik).to_numpy(), "MLP": du_bao, "naive": Xk[:, -1]}, index=ik)
    return kq.sort_index()


def danh_gia_gia(kq: pd.DataFrame) -> dict:
    """Độ chính xác của mô hình."""
    from sklearn.metrics import r2_score
    return {"R² MLP": r2_score(kq["y"], kq["MLP"])}
...
def var_99(r: pd.Series, moc: str = MOC_JPY) -> pd.Series:
    """VaR 99% một ngày (%, số âm): phân phối chuẩn với trung bình và độ lệch chuẩn của giai đoạn học."""
    hoc = r[r.index < pd.Timestamp(moc)]
    q = stats.norm.ppf(MUC_VAR)
    ngay = r.index[r.index >= pd.Timestamp(moc)]
    return pd.Series(hoc.mean() + q * hoc.std(), index=ngay, name="VaR")
